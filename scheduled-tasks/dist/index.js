...
async function getCourses() {
  ...
  `;
    const variables = {};
    try {
        const { data: { computed_listing_info: courses } } = await fetchCourseTable(query, variables);
        return courses;
    }
    catch (err) {
        console.error(err);
...
async function getEvaluationNarratives() {
    ...
    const variables = {};
    try {
        const { data: { evaluation_narratives } } = await fetchCourseTable(query, variables);
...
# 				section
    ...
    try {
        const { data: { evaluation_narratives } } = await fetchCourseTable(query, variables);
        const flattenedResults = evaluation_narratives.map(({ course: { computed_listing_infos: [course] }, ...rest }) => ({ ...rest, ...course }));
